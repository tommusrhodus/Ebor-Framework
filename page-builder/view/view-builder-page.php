...
switch($action) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'ebor-framework') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'ebor-framework') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'ebor-framework') . '</p>' . $errors . '</div>';
		}
	...
	case 'clone' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'ebor-framework') . '<strong>' . $template_name . '</strong>' . __(' template has been successfully cloned', 'ebor-framework') . '</p></div>';
		} else {
			$errors = '<ul>';
			foreach( $new_id->get_error_messages() as $error ) {
				$errors .= '<li><strong>'. $error . '</strong></li>';
	...
	case 'update' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'ebor-framework') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'ebor-framework') . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'ebor-framework') . '</p></div>';
		break;
}
...
<div class="wrap">
	...
	<div id="manage-templates" class="manage-templates">
		...
		<?php if($templates) { ?>
			...
			<form method="get" action="<?php echo admin_url('themes.php') ?>" >
				...
				<label for="template">
					<?php _e('Select a template to edit:', 'ebor-framework') ?> 

					<select name="template" id="">
						
						<?php if($selected_template_id === 0) { ?>
							<option value="0" selected="selected"><?php _e('-- Select --', 'ebor-framework') ?></option>
						<?php } ?>
				...
				<span class="add-new-template-action">
					...
					?>
					"><?php _e('Create a new template', 'ebor-framework') ?></a>
				</span>
		...
		<?php } else { ?>
			<span class="add-new-template-action">
				<?php _e('Create your new template below', 'ebor-framework') ?>
			</span>
		<?php } ?>
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-fixed">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label howto" for="template-name">
											<span><?php _e('Template Name', 'ebor-framework') ?></span>
											<input name="template-name" id="template-name" type="text" class="template-name regular-text" title="Enter template name here" placeholder="<?php _e('Enter template name here', 'ebor-framework') ?>" value="<?php echo is_object($selected_template_object) ? $selected_template_object->post_title : ''; ?>">
										</label>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'ebor-framework' ) : __( 'Save Template', 'ebor-framework' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'ebor-framework');
									echo '</p>';
							...
							<div class="major-publishing-actions cf">
								...
								<div class="delete-action">
									...
									echo '<a class="submitdelete deletion template-delete" href="' . esc_url(add_query_arg(
										...
										admin_url( 'themes.php' )
									)) . '">'. __('Delete Template', 'ebor-framework') .'</a>';
									?>
                                ...
                                <div class="clone-action">
                                    ...
                                    echo '<a class="submitclone cloning template-clone" href="' . esc_url(add_query_arg(
                                        ...
                                        admin_url( 'themes.php' )
                                    )) . '">'. __('Clone Template', 'ebor-framework') .'</a>';
                                    ?>
			...
			<p style="float:left"><small>Ebor Page Builder &copy; <?php echo date("Y") ?> by <a href="http://www.madeinebor.com">Tom Rhodes</a> & <a href="http://aquagraphite.com">Syamil MJ</a></small></p>
