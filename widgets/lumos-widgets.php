...
if(!( class_exists('ebor_lumos_popular_Widget') )){
	class ebor_lumos_popular_Widget extends WP_Widget {
		...
		{
			$widget_ops = array('classname' => 'ebor_lumos_popular', 'description' => '');
	
			$control_ops = array('id_base' => 'ebor_lumos_popular-widget');
	
			$this->WP_Widget('ebor_lumos_popular-widget', 'TommusRhodus: Popular Posts', $widget_ops, $control_ops);
		}
		    	...
		    	<ul class="post-list">
			    	...
			    	<li>
			    		<figure class="frame pull-left">
			    		  <div class="icon-overlay"><a href="<?php the_permalink(); ?>"><span class="icn-more"></span><?php the_post_thumbnail('thumbnail'); ?> </a></div>
			    		</figure>
			    		<div class="meta"> <em><span class="date"><?php the_time( get_option('date_format') ); ?> </span>
			    		<span class="comments"><a href="<?php comments_link(); ?>"><?php comments_number( '0','1','%' ); ?> <i class="icon-chat-1"></i></a></span></em>
	...
	function ebor_framework_register_ebor_lumos_popular(){
	     register_widget( 'ebor_lumos_popular_Widget' );
	}
	add_action( 'widgets_init', 'ebor_framework_register_ebor_lumos_popular');
}
