...
add_shortcode( 'ryla_map_block', 'ebor_map_block_shortcode' );
...
function ebor_map_block_shortcode_vc() {
	vc_map( 
		array(
			"icon" => 'ryla-vc-block',
			"name" => esc_html__("Google Map", 'ryla'),
			"base" => "ryla_map_block",
			...
			"params" => array(
				...
				array(
					"type" => "textfield",
					"heading" => esc_html__("Map height", 'ryla'),
					"param_name" => "height",
				...
				array(
					"type" => "attach_image",
					"heading" => esc_html__("Map Marker Image", 'ryla'),
					"param_name" => "image"
				)
			)
		) 
